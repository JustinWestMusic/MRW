(function() {
	var ext,
		audioSource,
		targetSrc,
		myPlayer = document.querySelector("#videoPlayer"),
	...
	//PLAYS WHEN YOU CLICK PLAY AND PAUSES WHEN YOU CLICK PAUSE
	function playToggle() 
	{
	...
	function buildTracks(tracklist) 
	{
		...
		[].forEach.call(tracklist, function(track, index) 
		{
			...
			newTrack.dataset.track = track.trailer_name;
			trackLabel.innerHTML = track.trailer_description;
		...
		$('.trackHolder li').on('click', function() 
		{
			...
			else nextTrack = tracklist[0].trailer_name;
		});
	...
	function playTrack(whichTrack) 
	{
		...
		//audio/ isjust the folder name not the extention
		myPlayer.src = "trailers/" + whichTrack + ext;
